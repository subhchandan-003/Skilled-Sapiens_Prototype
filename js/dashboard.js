...
function loadStudyBuddies() {
    ...
    if (buddies.length > 0) {
        buddiesList.innerHTML = buddies.map(buddy => {
            ...
            const name = buddyUser?.name || 'Study Buddy';
            const avatarHtml = createInitialsAvatarHTML(name, 40);
            return `
                <div class="buddy-item ${buddy.status}">
                    ${avatarHtml}
                    <div class="buddy-info">
                        <h4>${name}</h4>
                        <span class="badge badge-${buddy.status === 'accepted' ? 'success' : 'warning'}">${buddy.status === 'accepted' ? 'Connected' : 'Pending'}</span>
                    </div>
